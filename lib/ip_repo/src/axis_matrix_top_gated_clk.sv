...
(
   ...
   ///////////////////////////////////////////
    ...
    wire bram_en = (!hw_busy) | clk_en_safe;
    ...
    //门控时钟改造：
    //增加时钟使能信号，根据控制寄存器控制时钟的启停，减少功耗
    wire clk_en_raw = slv_reg0[2]; //假设控制寄存器的第2位是时钟使能位;
    //安全使能信号，下降沿触发，确保使能跳变在时钟低电平。
    logic clk_en_safe;
    always_ff @(negedge S_AXI_ACLK or negedge S_AXI_ARESETN)begin
        if(!S_AXI_ARESETN)
            clk_en_safe <= 1'b1;//复位时使能时钟，使内核也能重置
        else clk_en_safe <= clk_en_raw;
    end
    //例化BUFGCE门控时钟
    wire gated_clk;
    BUFGCE u_bufgce_mac(
        .I(S_AXI_ACLK),//输入全局时钟
        .CE(clk_en_safe),//时钟使能信号
        .O(gated_clk)//输出门控时钟
    );
    ...
    datapath u_datapath(
        ...
        //controller控制信号
        .acc_clear(acc_clear),
        .data_x_0(x_dout_0), //从BRAM_X读出的矩阵X元素
        .data_x_1(x_dout_1),
        .data_a_0(a_dout_0), //从BRAM_A读出的矩阵A元素
        .data_a_1(a_dout_1),
